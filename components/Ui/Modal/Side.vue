...
<template>
  <Transition :name="`fade-${position}`">
    <div
      v-if="modelValue"
      class="fixed w-full h-screen bg-black/20 top-0 left-0 z-[1000] sheet-wrapper"
      :class="{
    ...
    >
      <div
        class="flex bg-bg-primary  h-full sheet"
        :class="{
      ...
      >
        <div class="inner w-full relative">
          ...
          <slot name="header">
            <div
              class="flex items-center w-full p-5 border-b "
              :class="
                position === 'left'
                  ? 'justify-between'
            ...
            >
              ...
              <slot name="close">
                <button
                  class="text-text-primary  border rounded-xl  p-1"
                  @click="handleClose"
                >
                  <svg
          ...
          <!-- Main Slot  -->
          <div
            class="h-full overflow-y-scroll p-5 bg-bg-primary  pb-32 sheet-body"
          ...
          <div
            v-if="$slots.footer"
            class="footer fixed gap-3 sm:absolute bottom-0 sm:bottom-8 xl:bottom-0 right-0 left-0 bg-bg-primary  px-4 py-2 flex justify-end border-t z-50"
          >
